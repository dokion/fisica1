# CLAUDE.md — Orquestrador do Curso FIS-01

Este projeto publica o material do curso de Física (FIS-01 — Escola de formação de
...
| `FIS_1_RA.md` | RAs já extraídos e formatados | **Fonte oficial dos RAs de FIS-01.** NÃO reinventar nem reordenar RAs — usar estes. |
...
6. **RAs de FIS-01** → `FIS_1_RA.md` é a fonte. Não gerar RA novo a menos que o pedido seja explicitamente "criar RA" via Apêndice III.
...
- **RAs de FIS-01:** vêm de `FIS_1_RA.md`. Não inventar.
...
- **Estrutura por tópico do sumário (decisão local — substitui o nome `aula-NN` do Q2):**
  ...
  oficial (`_docs/04. FIS-1.pdf`), tópico com zero à esquerda (`1-01`, `4-16`). O comentário
  no topo de cada arquivo lista os objetivos (OE) do tópico, copiados de `_docs/FIS_1_RA.md` —
